...
for (Tr in 1:N_triads){
  ...
  for (r in 1:reps){
    ...
    for (H in 1:N_hab_triads){
      ...
      meta_monad_H <- subset(FV_monads, Habitat == T_habitats[H])
      ...
      int2sample <- sample(nrow(meta_monad_H), size = N_TH, prob = meta_monad_H$n_int, replace = TRUE)
      null_subtriad_H <- subset(meta_monad_H[int2sample, ], select = c(Habitat, Lower_Taxon, Upper_Taxon))
...
print('null models finished')
# export null model results
write.csv(null_metrics, paste(OUT_DIR, paste0("null_model_1_metrics_", task_id, ".csv"), sep = "/"), quote = FALSE, row.names = FALSE)

elapsed_time <- toc(quiet = TRUE); elapsed_time <- elapsed_time$toc - elapsed_time$tic
exec_time <- data.frame(nm = "nm_1", ID = task_id, time = elapsed_time)
