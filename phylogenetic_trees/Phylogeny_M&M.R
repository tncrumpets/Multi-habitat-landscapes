...
rm(list = ls()); getwd()
setwd("C:/Users/user/Dropbox/Kate/Talya_FD/Data")
interac_data <- read.table("all_web_interactions_final.txt", header = TRUE, sep = "\t")
...
phyl_data <- read.table("DaPhnE_02_Synonymy.txt", header = TRUE, sep = "\t", quote = "") # lines 4847:4849 were a mistake and removed in the dataset 
...
# write.table(data.frame(miss_plant_list), "miss_plant_list.txt", sep = "\t")
...
miss_plant_list <- read.table("miss_plant_list.txt", header = TRUE, sep = "\t", na.strings = "")
...
df <- df[,-c(3,5,6)]
# write.table(df, "alternative_daphne.txt", sep = "\t")
# Added to lant_solution.xlsx and used for tasks b and c.
...
likely <- read.csv("likely_plant_species_search.csv") # created from df
sites <- read.table("sites_coords.txt", header = TRUE, sep = "\t", quote = "\"")
gbif <- read.csv("GBIFDownl_0037864-200221144449610.csv", header = TRUE, sep = "\t")
...
# Function to be used - saved here to avoid encoding problems
fix_coord <- function(coords) {
  
  seq1 <- seq(1, length(coords)*2, 2)
  seq2 <- seq(2, length(coords)*2, 2)
  
  deg <- unlist(strsplit(as.character(coords), "°"))[seq1]
  rest_d <- factor(unlist(strsplit(as.character(coords), "°"))[seq2])
  
  min <- unlist(strsplit(as.character(rest_d), "'"))[seq1]
  rest_m <- factor(unlist(strsplit(as.character(rest_d), "'"))[seq2])
  
  sec <- unlist(strsplit(as.character(rest_m), "\""))[seq1]
  sec <- round(as.numeric(sec), digits = 0)
  
  let <- unlist(strsplit(as.character(rest_m), "\""))[seq2]
  
  result <- paste(deg, "d", min, "m", sec, "s", let, sep = "")
  return(result)
}

# Site coordinates: from DMS to DD
fix_lat <- fix_coord(sites$Latitude)
fix_lat <- char2dms(fix_lat, chd = "d", chm =  "m",chs =  "s")
sites$fix_lat <- as.numeric(fix_lat)

fix_lon <- fix_coord(sites$Longitude)
fix_lon <- char2dms(fix_lon, chd = "d", chm =  "m",chs =  "s")
sites$fix_lon <- as.numeric(fix_lon)
...
# write.table(miss_plant_list, "plant_solution.txt", sep = "\t", row.names = FALSE)
