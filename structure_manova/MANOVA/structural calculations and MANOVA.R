...
allinsectspersite <-
  aggregate(allwebdata_no0$Upper_Taxon_abundance,
            ...
            length)
write.table(allinsectspersite, file = "number of total insects per site.txt", 
            sep ="\t")
...
write.table(number_of_insect_sp_persite,
            "total_insect species richness per site.txt",
            sep = "\t")

detach(allwebdata)

site_data <- read.table("Site level data.txt", header = T, sep = "\t")
insects_persite <-
  as.data.frame(cbind(
    site_data$MDT,
    allinsectspersite$x,
...
write.table(floral_units, "floral units per sites.txt", sep = "\t")
...
allwebdata_no0 <-
  subset(allwebdata, allwebdata$Upper_Taxon_abundance != "")
siteleveldata <- read.table("Site level data.txt", header = T, sep = "\t")
...
write.table(speciesevenness, "species evenness insects and plants.txt", sep =
              "\t")
...
write.table(IE_all_insects, file = "Interaction Evenness_all webs.txt", sep =
              "\t")
